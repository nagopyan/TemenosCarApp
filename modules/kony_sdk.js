        ...
        try {
            ...
            if (konyRef.internalSdkObject) {
                ...
                if (konyRef.internalSdkObject.setClientParams) {
                    if (appConfig) {
                        konyRef.internalSdkObject.setClientParams({"aid": appConfig.appId, "aname": appConfig.appName, "aversion": appConfig.appVersion});
                    } else {
...
function setIntegrityParamsHandler(){
    setIntegrityParams();
    try {
        if(!(kony.sdk.getPlatformName() === kony.sdk.constants.PLATFORM_WINDOWS || kony.sdk.getAType() === kony.sdk.constants.SDK_ATYPE_SPA || kony.sdk.getAType() === "watch")){
...
function IdentityService(konyRef, rec) {
	...
	function _logout(successCallback, failureCallback, options) {
		...
		function logoutSuccess(data) {
			...
			if (doesSessionNeedsToBeCleanedAtMW) {
				...
				var middlewareSessionInvalidationURL = konyRef[kony.sdk.constants.INTEGRATION_SERVICE_KEY]
						[kony.sdk.constants.INTEGRATION_INTERNAL_LOGOUT_URL]
				...
				networkProvider.post(middlewareSessionInvalidationURL,
									 ...
									 function(errorResponseFromMW) {
										 ...
										 kony.sdk.logsdk.error(
											 ...
											 errorResponseFromMW[kony.sdk.constants.MF_ERROR_MSG]);
        ...
        else if ((_type == kony.sdk.constants.AUTH_PROVIDER_TYPE_OAUTH2 || _type == kony.sdk.constants.AUTH_PROVIDER_TYPE_SAML)
            && kony.sdk.getSdkType() == kony.sdk.constants.SDK_TYPE_IDE) {
            ...
            function oAuthCallback(status){
                ...
                if(callback_invoke) {
                    ...
                    try {
                        ...
                        kony.sdk.logsdk.warn("logout is already in progress.");
                    }
                }
            }
			var oauthOptions = {};
...
kony.sdk.util.checkAndFetchNetworkProviderOptions = function (options, isPassThroughExpected) {
    var providerOptions = {};
    if(!kony.sdk.isNullOrUndefined(options)) {
        if (!kony.sdk.isNullOrUndefined(options["httpRequestOptions"]) &&
                options["httpRequestOptions"] instanceof Object) {
            providerOptions["httpRequestOptions"] = options["httpRequestOptions"];
        }
        if (!kony.sdk.isNullOrUndefined(options["xmlHttpRequestOptions"]) &&
...
kony.sdk.util.isPWAStandaloneOrFullscreen = function() {
    if(!kony.sdk.isNullOrUndefined(window)
        && window.matchMedia
...
kony.sdk.clientCertificatesManager = (function() {
    var instance = null;
    var LOG_PREFIX = "kony.sdk.clientCertificatesManager";

    function createInstance() {
        kony.sdk.logsdk.info(LOG_PREFIX + ": Creating instance of KNYClientCertificatesManager");
        var obj = {};
    ...
    return {
        import : function() {
            kony.sdk.logsdk.info(LOG_PREFIX + ": Importing native classes for SSL Pinning");
            if(instance === null){
                instance = createInstance();
            }
            return instance;
...
* API - kony.net.loadClientCertificate
*/
var konyNetLoadClientCertificates = kony.net.loadClientCertificate;

kony.net.loadClientCertificate = function(certParams) {

    function konySDKLoadClientCertificate(certParams) {
        var LOG_PREFIX = "kony.SDK.loadClientCertificate";
        kony.sdk.logsdk.trace("Entering "+ LOG_PREFIX);

        var importedClasses = kony.sdk.clientCertificatesManager.import();
        return importedClasses.clientCertificatesManager.loadClientCertificate(certParams);
    }

    var cert = "cert";
    if(kony.type(certParams[cert]) === "kony.types.RawBytes") {
    ...
    //Calling SDK's API
    var certParamsHashMap = kony.sdk.OfflineObjects.createHashMapFromJSONObject(certParams, "Cert Params for SSL Pinning");
    konySDKLoadClientCertificate(certParamsHashMap);
    // Calling Framework's API
    return konyNetLoadClientCertificates(certParams);
}

/*
* Overriding the logic of frameworks implementation to support 2-way SSL Pinning
* API - kony.net.removeClientCertificate
*/
var konyNetRemoveClientCertificate = kony.net.removeClientCertificate;

kony.net.removeClientCertificate = function() {

    function konySDKRemoveClientCertificate() {
        var LOG_PREFIX = "konySDKRemoveClientCertificate";
        kony.sdk.logsdk.trace("Entering "+ LOG_PREFIX);
...
kony.sdk.OfflineObjects.setup = function (objServiceList, options, successCallback, failureCallback) {
	...
	var syncCallback = kony.sdk.OfflineObjects.createSyncCallback("Setup Success", successCallback, "Setup Failed", onFailure);
	var syncV2Classes = kony.sdk.SyncV2Classes.import();
	...
	function onFailure(error) {
		if(error && error.userInfo && error.userInfo.setupStatus) {
			error.setupStatus = kony.sdk.OfflineObjects.createJSONObjectFromHashMap(error.userInfo.setupStatus, "Setup : FailureCallbackObject");
		}
...
kony.sdk.OfflineObjects.reset = function (objServiceList, options, successCallback, failureCallback) {
	...
	var syncCallback = kony.sdk.OfflineObjects.createSyncCallback("Reset Success", successCallback, "Reset Failed", onFailure);
	var syncV2Classes = kony.sdk.SyncV2Classes.import();
	...
	function onFailure(error) {
		if(error && error.userInfo && error.userInfo.setupStatus) {
			error.setupStatus = kony.sdk.OfflineObjects.createJSONObjectFromHashMap(error.userInfo.setupStatus, "Reset : FailureCallbackObject");
		}
...
kony.sdk.KNYObj.clearOfflineData = function (knyObj, options, successCallback, failureCallback) {
	var LOG_PREFIX = "kony.sdk.KNYObj.clearOfflineData";
	kony.sdk.logsdk.trace(" Entering " + LOG_PREFIX);
	var optionsMap = kony.sdk.OfflineObjects.createHashMapFromJSONObject(options, "clearOfflineData options");
	var syncCallback = kony.sdk.OfflineObjects.createSyncCallback("Successfully cleared offline data in " + knyObj.name + " object.", onSuccess, "Failed to clear offline data in " + knyObj.name + " object.", onFailure);
	var sdkObjectSync = knyObj.getSdkObjectSync();
...
kony.sdk.clientCertificatesManager = (function() {
    ...
    return {
        import : function(){
            kony.sdk.logsdk.info(LOG_PREFIX+": Importing native classes for KNYClientCertificatesManager.");
            if(instance === null){
                instance = createInstance();
            }
            return instance;
...
kony.net.loadClientCertificate = function(certParams) {
    ...
    function konySDKLoadClientCertificate(certParams) {
        var LOG_PREFIX = "konySDKLoadClientCertificate";
        kony.sdk.logsdk.trace("Entering "+ LOG_PREFIX);
    ...
    //Calling SDK's API
    konySDKLoadClientCertificate(certParams);
    // Calling Framework's API
    return konyNetLoadClientCertificates(certParams);
}
...
kony.sdk.KNYObj.clearOfflineData = function (knyObj, options, successCallback, failureCallback) {
	var LOG_PREFIX = "kony.sdk.KNYObj.clearOfflineData";
	kony.sdk.logsdk.trace("Entering " + LOG_PREFIX);
	var syncCallback = kony.sdk.OfflineObjects.createSyncCallback("Successfully cleared offline data in " + knyObj.name + " object .", successCallback, "Failed to clear offline data in " + knyObj.name + " object.", failureCallback);
	var sdkObjectSync = knyObj.getSdkObjectSync();
...
kony.sdk.prototype.getMessagingService = function() {
    ...
    if ((this.messagingsvc.url !== KNYMessagingService.getUrl()) ||
        (this.messagingsvc.appId !== KNYMessagingService.getKmsAppId())) {
...
function MessagingService(konyRef) {
    ...
    */
        ...
        if(data.state.toLocaleUpperCase() === "ENTRY" || data.state.toLocaleUpperCase() === "ENTER"){
            if(data.geofenceID !== "refreshBoundary"){
            	...
            	if(action && action.clientAction === "notifyEngagementServer") {
                    KNYMessagingService.updateGeoLocation(data.lat, data.lon, action.locationName, function (res) {
                    ...
                    if(!kony.sdk.isNullOrUndefined(geoBoundariesOptions["customLogicCallback"]) && typeof (geoBoundariesOptions["customLogicCallback"]) == kony.sdk.constants.FUNCTION_STRING) {
                        ...
                        kony.sdk.verifyAndCallClosure(geoBoundariesOptions["customLogicCallback"],action);
                    }else{
			...
			var options = {};
			options[kony.sdk.constants.AUTH_TOKEN] = KNYMessagingService.getAuthToken();
			if(data.geofenceID === "refreshBoundary"){
                KNYMessagingService.updateGeoLocation(data.lat, data.lon, data.geofenceID, function (res) {
                    kony.sdk.logsdk.info("MessagingService::manageGeoBoundariesCallback successfully notified KMS");
                    getAndRefreshBoundaries(geoBoundariesOptions, function(res1){
                        ...
                        kony.sdk.verifyAndCallClosure(KNYMessagingService.refreshBoundariesFailureCallback, err1);
                    }, options);
                }, function (err) {
                    kony.sdk.logsdk.error("MessagingService::manageGeoBoundariesCallback error in notifying KMS");
                });
    ...
    var getAndRefreshBoundaries = function (geoBoundaryOptions, successCallback, failureCallback, options) {
		kony.sdk.logsdk.perf("Executing getAndRefreshBoundaries");
        ...
        var networkOptions={};
        networkOptions[kony.sdk.constants.DISABLE_INTEGRITY] = true;
		kony.sdk.logsdk.perf("Executing getAndRefreshBoundaries's network call");
		networkProvider.get(url, null, headers,
			function(res){
				kony.sdk.logsdk.perf("Executing finished getAndRefreshBoundaries with network success");
				var geoBoundaries = [];
				var boundaryActions = {};
				var boundariesLimit;
                //override data store auth token with user given token
                ...
                // kony.location.setGeofencesCallback method should be called in PreAppInit for IOS to receive the callbacks. Other platforms do not have such rule.
                if(kony.sdk.getPlatformName() !== kony.sdk.constants.PLATFORM_IOS){
                    kony.location.setGeofencesCallback(
                        kony.sdk.getCurrentInstance().getMessagingService().manageGeoBoundariesCallback
                    );
                }
	...
	this.registerGeoBoundaries = function(options, successCallback, failureCallback){
		kony.sdk.logsdk.perf("Executing registerGeoBoundaries");
        var tempKSID = currentObject.getKSID();
        ...
        if(!kony.sdk.isNullOrUndefined(options)) {
            ...
            if(!kony.sdk.isNullOrUndefined(options["customLogicCallback"]) && typeof (failureCallback) == kony.sdk.constants.FUNCTION_STRING){
        ...
        KNYMessagingService.setGeoBoundariesOptions(geoBoundaryOptions);
        ...
        if(!typeof (failureCallback) == kony.sdk.constants.FUNCTION_STRING){
            kony.sdk.logsdk.perf("Executing finished registerGeoBoundaries with an exception");
        ...
        kony.location.getCurrentPosition(
        	function(res){
        		...
        		updateGeoLocationForMessagingService(currentLocation.latitude,
					...
					authToken,
					function(res){
                        kony.sdk.logsdk.perf("Executing finished registerGeoBoundaries with success");
					...
					function(err){
                        kony.sdk.logsdk.perf("Executing finished registerGeoBoundaries with a failure");
...
function OAuthHandler(serviceUrl, providerName, appkey, callback, type, options, isMFVersionCompatible) {
    ...
    function constructURLIE11(crossPlatformBaseURL, identityOAuthUrl) {
        ...
        var finalRequestUrl = crossPlatformBaseURL + "/" + kony.sdk.constants.KNY_OAUTH_REDIRECT_HTML + "?"
            ...
            + kony.sdk.constants.KNY_OAUTH_PROTOCOL_PARAM + "=" + protocolParam;
...
kony.sdk.util.getSSOToken = function(){
    if(typeof(SSOFFI) !== "undefined" ) {
        ...
        return kony.sdk.util.decryptSSOToken(decryptedToken);
    } else {
        return null;
    }
};
...
kony.sdk.util.saveSSOToken = function () {
    kony.sdk.logsdk.warn("SSO is not currently supported for windows platform.");
};
kony.sdk.util.getSSOToken = function () {
    kony.sdk.logsdk.warn("SSO is not currently supported for windows platform.");
};
...
kony.setupsdks = function (initConfig, successCallBack, errorCallBack) {
    ...
    function getAppConfigFromCache() {
        ...
        if (!kony.sdk.isNullOrUndefined(dsAppMetaData)) {
            if (kony.sdk.util.isAppConfigEncrypted()) {
                ...
                if (!kony.sdk.isNullOrUndefined(appKey)
                    || !kony.sdk.isNullOrUndefined(appSecret)
                    || !kony.sdk.isNullOrUndefined(serviceUrl)
                ...
                if (!kony.sdk.isNullOrUndefined(dsAppServiceDoc)) {
                    ...
                    if (!kony.sdk.isNullOrUndefined(decryptedSvcDoc)) {
                        try {
                            ...
                            kony.sdk.logsdk.debug("Failed to retrieve config data form Cache : ", err.toString());
                            return null;
                        }
                    } else {
                        kony.sdk.logsdk.debug("Failed to retrieve config data form Cache");
    ...
    if (!kony.sdk.util.isNullOrUndefinedOrEmptyObject(cachedAppConfig)) {
        ...
        if ((cachedMfAppMetaData.appKey === initConfig.appKey)
            ...
            if (cachedSvcDocEtagDateTime > toolsBundledSvcDocEtagDateTime) {
                kony.sdk.updateAppConfigWithDsAppMetaData(initConfig, cachedMfAppMetaData, cachedSvcDoc);
    ...
    var initKNYMobileFabric = function (initConfig) {
        ...
        clientParams.aversion = appConfig.appVersion;
    ...
    var sdkInit = function (initConfig, successcallback, failurecallback) {
        ...
        var refreshServiceDoc = function () {
            ...
            networkProvider.get(initConfig.serviceUrl, null, headers,
                function (data) {
                    kony.sdk.dataStore.setItem(kony.sdk.util.prefixAppid(kony.sdk.constants.MOBILE_FABRIC_SERVICE_DOC), kony.sdk.util.encryptAppConfig(JSON.stringify(data)));
                },
                function (data) {
        ...
        if (!initConfig.isMFApp) {
            ...
            if (isInvalidConfig || initConfig.appConfig.svcDocRefresh) {
                var cachedServiceDoc = "";

                if (kony.sdk.util.isAppConfigEncrypted()) {
                    cachedServiceDoc = kony.sdk.util.decryptAppConfig(kony.sdk.dataStore.getItem(kony.sdk.util.prefixAppid(kony.sdk.constants.MOBILE_FABRIC_SERVICE_DOC)));
                } else {
                    cachedServiceDoc = kony.sdk.dataStore.getItem(kony.sdk.util.prefixAppid(kony.sdk.constants.MOBILE_FABRIC_SERVICE_DOC));
                }
...
function konyNetHttpRequest(url, params, headers, httpMethod, konyContentType, successCallback, failureCallback, options) {
    ...
    function localRequestCallback(result) {
        ...
        if (readyState === 4) {
            ...
            if (options && options[kony.sdk.constants.PASSTHROUGH] && kony.sdk.convertPassthroughResponseToJson !== true) {
    ...
    if ((typeof(konyRef) !== "undefined" && konyRef && konyRef.mainRef.integrityKey === true)
        || (!kony.sdk.isNullOrUndefined(options) && options.isAppConfigCall === true)) {
        ...
        if ((options && (options[kony.sdk.constants.DISABLE_INTEGRITY] || options[kony.sdk.constants.PASSTHROUGH]))
            || ((typeof(konyRef) !== "undefined" && konyRef && konyRef.mainRef.integrityKey === false)
                || (!kony.sdk.isNullOrUndefined(options) && options.isAppConfigCall === false))) {
...
kony.sdk.getPlatformName = function () {
    var returnVal = kony.sdk.constants.PLATFORM_WINDOWS;
    //#ifdef PLATFORM_IOS
    ...
    //SPA platform should be added as thinclient
    return returnVal;
};
...
/**
 * Deleting App metadata from datastore
 */
...
kony.sdk.updateAppConfigWithDsAppMetaData = function (initConfig, dsAppMetaData, dsAppServiceDoc) {
    initConfig.appConfig.appKey = dsAppMetaData.appKey;
    initConfig.appConfig.appSecret = dsAppMetaData.appSecret;
    initConfig.appConfig.serviceUrl = dsAppMetaData.serviceUrl;
    initConfig.appConfig.isturlbase = dsAppServiceDoc.reportingsvc.session;
    initConfig.appConfig.svcDoc = dsAppServiceDoc;
...
kony.sdk.util.encryptText = function (text, salt, encryptionAlgo) {
    try {
        ...
        if(kony.sdk.isNullOrUndefined(encryptedText) || encryptedText == "") {
            kony.sdk.util.recordCustomEvent("INVALID APP CONFIG", "encryptedText: " + encryptedText, "encryptedText: null or EMPTY", "cipher", null);
        }
        var base64Data = kony.convertToBase64(encryptedText);
        if(kony.sdk.isNullOrUndefined(base64Data) || base64Data == "") {
            kony.sdk.util.recordCustomEvent("INVALID APP CONFIG", "base64Data: " + base64Data, "base64Data: null or EMPTY", "cipher", null);
        }
        return base64Data;
    } catch (exception) {
        kony.sdk.logsdk.error("Exception occurred while encrypting text, exception :", exception);
        kony.sdk.util.recordCustomEvent("INVALID APP CONFIG", "get encryptText: " + text, "encryptText: exception", "cipher", null);
    }
};
...
kony.sdk.util.convertBase64ToRawBytes = function (base64String) {
    //#ifdef PLATFORM_NATIVE_ANDROID_IOS_WINDOWS
    var retVal = kony.convertToRawBytes(base64String);
    //#else
...
kony.sdk.util.loadMetadataFromDs = function (dsAppMetaData, dsAppServiceDoc) {
    ...
    if (kony.sdk.util.isAppConfigEncrypted()) {
        if (!kony.sdk.isNullOrUndefined(dsAppMetaData)) {
            ...
            if (kony.sdk.util.isNullOrEmptyString(appKey) || kony.sdk.util.isNullOrEmptyString(appSecret)
                || kony.sdk.util.isNullOrEmptyString(serviceUrl)) {
...
kony.sdk.OfflineObjects.setToken = function(token){
    var LOG_PREFIX = "kony.sdk.OfflineObjects.setToken";
    kony.sdk.logsdk.trace("Entering "+LOG_PREFIX);
    kony.sdk.OfflineObjects.setCookie();
...
* */
kony.sdk.OfflineObjects.setCookie = function(){
    var LOG_PREFIX = "kony.sdk.OfflineObjects.setCookie";
	var fabricUrl = KNYMobileFabric.mainRef.config.selflink;
    var cookies = kony.net.getCookies(fabricUrl);
    if(!kony.sdk.isNullOrUndefined(cookies)) {
        var rawCookie = "";
        for (index = 0; index < cookies.length; index++) {
            rawCookie += cookies[index] + ";";
        }
        WindowsOfflineObjects.setCookie(fabricUrl,rawCookie);
    }
};
...
kony.sdk.KNYObj.update = function(knyObj, record, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.update";
    kony.sdk.logsdk.trace(" Entering "+LOG_PREFIX);
    kony.sdk.KNYObj.getWindowsOfflineSDKObject().update(knyObj.name.toString(), record, options, successCallback, failureCallback);
};
...
kony.sdk.KNYObj.delete = function(knyObj, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.delete";
    kony.sdk.logsdk.trace(" Entering "+LOG_PREFIX);
    kony.sdk.KNYObj.getWindowsOfflineSDKObject().delete(knyObj.name.toString(), options, successCallback, failureCallback);
};
...
kony.sdk.KNYObj.getBinaryStatus = function(knyObj, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.getBinaryStatus";
    kony.sdk.logsdk.trace("Entering "+ LOG_PREFIX);
    kony.sdk.logsdk.error("getBinaryStatus is not available for Windows OfflineObjects");
};
...
kony.sdk.KNYObj.markForUpload = function(knyObj, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.markForUpload";
    kony.sdk.logsdk.trace(" Entering "+LOG_PREFIX);
    kony.sdk.KNYObj.getWindowsOfflineSDKObject().markForUpload(knyObj.name.toString(), options, successCallback, failureCallback);
};
...
kony.sdk.KNYObj.cancelSync = function(knyObj, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.cancelSync";
    kony.sdk.logsdk.trace(" Entering "+LOG_PREFIX);
    kony.sdk.KNYObj.getWindowsOfflineSDKObject().cancelSync(knyObj.name.toString(),options, successCallback, failureCallback);
};
...
kony.sdk.KNYObj.getUploadDeferredRecordKeys = function(knyObj, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.getUploadDeferredRecordKeys";
    kony.sdk.logsdk.trace(" Entering "+LOG_PREFIX);
    kony.sdk.KNYObj.getWindowsOfflineSDKObject().getUploadDeferredRecordKeys(knyObj.name.toString(), successCallback, failureCallback);
};
...
kony.sdk.KNYObj.getPendingRecordsForUpload = function(knyObj, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.getPendingRecordsForUpload";
    kony.sdk.logsdk.trace(" Entering "+LOG_PREFIX);
    kony.sdk.KNYObj.getWindowsOfflineSDKObject().getPendingRecordsForUpload(knyObj.name.toString(),options, successCallback, failureCallback);
};
...
kony.sdk.KNYObj.clearOfflineData = function(knyObj, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObj.clearOfflineData";
    kony.sdk.logsdk.trace(" Entering " + LOG_PREFIX);
    kony.sdk.KNYObj.getWindowsOfflineSDKObject().clearOfflineData(knyObj.name.toString(), options, successCallback, failureCallback);
};
...
kony.sdk.KNYObjSvc.cancelSync = function(knyObjSvc, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObjSvc.cancelSync";
    kony.sdk.logsdk.trace(" Entering "+LOG_PREFIX);
    kony.sdk.KNYObjSvc.getWindowsOfflineSDKObjectService().cancelSync(knyObjSvc.name.toString(), options, successCallback, failureCallback);
};
...
kony.sdk.KNYObjSvc.clearOfflineData = function(knyObjSvc, options, successCallback, failureCallback){
    ...
    var LOG_PREFIX = "kony.sdk.KNYObjSvc.clearOfflineData";
    kony.sdk.logsdk.trace(" Entering " + LOG_PREFIX);
    kony.sdk.KNYObjSvc.getWindowsOfflineSDKObjectService().clearOfflineData(knyObjSvc.name.toString(), options, successCallback, failureCallback);
};
